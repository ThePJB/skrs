...
impl CampaignMenu {
    ...
    pub fn frame(&mut self, inputs: &FrameInputState, rc: &mut Vec<RenderCommand>) {
        ...
        if let Some(instance) = self.game_instance.as_mut() {
            if instance.frame(inputs, rc) {
                ...
                if self.level_selection as usize == self.worlds[self.world_selection as usize].levels.len() - 1 {
                    ...
                    self.game_instance = Some(Instance {
                        level: self.worlds[self.world_selection as usize].levels[self.level_selection as usize].clone(),
                    });
                }
            }
        } else {
            ...
            rc.push(RenderCommand::solid_rect(menu_rect, Vec4::new(1.0, 0.0, 0.0, 1.0), 1.0));
            for i in 0..self.worlds.len() {
                let world_rect = menu_rect.grid_child(0, i as i32, 1, self.worlds.len() as i32);
